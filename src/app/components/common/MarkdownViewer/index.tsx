    ...
    <div className={`markdown-viewer prose prose-gray max-w-none ${className}`} {...props}>
      <ReactMarkdown
        components={{
          ...
          h2: (props) => <h2 className="text-2xl font-bold mt-6 mb-3 text-gray-dark border-b pb-1 border-gray-200" {...props} />,
          ...
          blockquote: (props) => <blockquote className="border-l-4 border-primary pl-4 italic my-4 text-gray-light" {...props} />,
          code: (props) => {
            const { className, children } = props;
            const isInline = !className || !className.startsWith('language-');
            return isInline 
              ? <code className="bg-bg-light px-1 py-0.5 rounded text-primary">{children}</code>
              : <code className="block bg-bg-light p-4 rounded text-gray-dark overflow-x-auto my-4">{children}</code>;
          },
